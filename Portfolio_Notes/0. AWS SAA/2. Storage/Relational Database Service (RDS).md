...
- Backed by [[1.Elastic Compute Cloud (EC2)|EC2]] instances with [[2.Elastic Block Storage (EBS)|EBS]] storage
...
- Allows us to scale the read operation (SELECT) on RDS
	- ![[Pasted image 20220507005920.png]]
...
- Increase availability of the RDS database by replicating it to another AZ
	- ![[Pasted image 20220507010002.png]]
...
- EC2 instances & [[4.Lambda]] functions should access the DB using **IAM DB Authentication** (**AWSAuthenticationPlugin** with **IAM**) - token based access
	- EC2 instance or Lambda function has an IAM role which allows is to make an API call to the RDS service to get the **auth token** which it uses to access the MySQL database.
		- ![[Pasted image 20220507011632.png]]

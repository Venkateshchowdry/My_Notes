...
## Plugins and their usages
#### 1. Publish over ssh
- used for establishing ssh connection over servers
#### 2. ThinBackup 
- used to back up jobs if jenkins crashes
- -->go to manage jenkins-->scroll down
- -->select thinBackup-->settings
- -->backup directory : /tmp/backup1
	- we have different option for back up choose according to need
		- -->backup plugin archives
		- -->move old backups to zip files
- -->save
	- -->backup now -->to back up the jobs
- we see the files stored in the backup folder path we specified

## Tips
- if some one asks the job of jenkins to run on another jenkins server
	- give the config.xml file inside the 
		- scp -r /var/lib/jenkins/workspace/jobname/config.xml root@[ip]:/ path
- go to the second jenkins server
	- -->manage jenkin
	- -->reload configuration from desk
		- -->ok
	- done 
		- we will see the job
- 